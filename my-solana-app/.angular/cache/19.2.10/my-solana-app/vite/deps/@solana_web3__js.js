...
function TextEncoder2(encoding, options) {
  if (!(this instanceof TextEncoder2)) return new TextEncoder2(encoding, options);
  encoding = encoding !== void 0 ? String(encoding).toLowerCase() : DEFAULT_ENCODING;
  if (encoding !== DEFAULT_ENCODING) {
    throw new Error("Encoding not supported. Only utf-8 is supported");
  }
...
function UTF8Encoder(options) {
  ...
  this.handler = function(stream, code_point) {
    ...
    if (inRange2(code_point, 0, 127)) return code_point;
    var count, offset2;
...
var SHA256_IV = Uint32Array.from([1779033703, 3144134277, 1013904242, 2773480762, 1359893119, 2600822924, 528734635, 1541459225]);
var SHA224_IV = Uint32Array.from([3238371032, 914150663, 812702999, 4144912697, 4290775857, 1750603025, 1694076839, 3204075428]);
var SHA384_IV = Uint32Array.from([3418070365, 3238371032, 1654270250, 914150663, 2438529370, 812702999, 355462360, 4144912697, 1731405415, 4290775857, 2394180231, 1750603025, 3675008525, 1694076839, 1203062813, 3204075428]);
var SHA512_IV = Uint32Array.from([1779033703, 4089235720, 3144134277, 2227873595, 1013904242, 4271175723, 2773480762, 1595750129, 1359893119, 2917565137, 2600822924, 725511199, 528734635, 4215389547, 1541459225, 327033209]);
...
var SHA256_K = Uint32Array.from([1116352408, 1899447441, 3049323471, 3921009573, 961987163, 1508970993, 2453635748, 2870763221, 3624381080, 310598401, 607225278, 1426881987, 1925078388, 2162078206, 2614888103, 3248222580, 3835390401, 4022224774, 264347078, 604807628, 770255983, 1249150122, 1555081692, 1996064986, 2554220882, 2821834349, 2952996808, 3210313671, 3336571891, 3584528711, 113926993, 338241895, 666307205, 773529912, 1294757372, 1396182291, 1695183700, 1986661051, 2177026350, 2456956037, 2730485921, 2820302411, 3259730800, 3345764771, 3516065817, 3600352804, 4094571909, 275423344, 430227734, 506948616, 659060556, 883997877, 958139571, 1322822218, 1537002063, 1747873779, 1955562222, 2024104815, 2227730452, 2361852424, 2428436474, 2756734187, 3204031479, 3329325298]);
...
var SHA256 = class extends HashMD {
  ...
  process(view, offset2) {
    for (let i = 0; i < 16; i++, offset2 += 4) SHA256_W[i] = view.getUint32(offset2, false);
    for (let i = 16; i < 64; i++) {
...
var T224_IV = Uint32Array.from([2352822216, 424955298, 1944164710, 2312950998, 502970286, 855612546, 1738396948, 1479516111, 258812777, 2077511080, 2011393907, 79989058, 1067287976, 1780299464, 286451373, 2446758561]);
var T256_IV = Uint32Array.from([573645204, 4230739756, 2673172387, 3360449730, 596883563, 1867755857, 2520282905, 1497426621, 2519219938, 2827943907, 3193839141, 1401305490, 721525244, 746961066, 246885852, 2177182882]);
...
function bytesToHex(bytes) {
  abytes2(bytes);
  if (hasHexBuiltin2) return bytes.toHex();
  let hex = "";
...
function hexToBytes(hex) {
  if (typeof hex !== "string") throw new Error("hex string expected, got " + typeof hex);
  if (hasHexBuiltin2) return Uint8Array.fromHex(hex);
  const hl = hex.length;
  const al = hl / 2;
...
function invert(number2, modulo) {
  if (number2 === _0n2) throw new Error("invert: expected non-zero number");
...
function FpPow(Fp2, num2, power) {
  ...
  if (power === _0n2) return Fp2.ONE;
  if (power === _1n2) return num2;
  let p = Fp2.ONE;
  let d = num2;
  while (power > _0n2) {
    if (power & _1n2) p = Fp2.mul(p, d);
    d = Fp2.sqr(d);
    power >>= _1n2;
  }
  return p;
...
function twistedEdwards(curveDef) {
  ...
  class Point2 {
    ...
    multiplyUnsafe(scalar, acc = Point2.ZERO) {
      ...
      if (n === _0n4) return I;
      if (this.is0() || n === _1n4) return this;
...
function createHasher2(Point2, mapToCurve, defaults) {
  ...
  function clear(initial) {
    ...
    if (P.equals(Point2.ZERO)) return Point2.ZERO;
    P.assertValidity();
    return P;
  }
  return {
    ...
    hashToCurve(msg, options) {
      const u = hash_to_field(msg, 2, __spreadValues(__spreadProps(__spreadValues({}, defaults), {
        DST: defaults.DST
      }), options));
      const u0 = map(u[0]);
      const u1 = map(u[1]);
    ...
    encodeToCurve(msg, options) {
      const u = hash_to_field(msg, 1, __spreadValues(__spreadProps(__spreadValues({}, defaults), {
...
var ed25519_hasher = (() => createHasher2(ed25519.ExtendedPoint, (scalars) => map_to_curve_elligator2_edwards25519(scalars[0]), {
...
var RistPoint = class _RistPoint {
  ...
  static fromHex(hex) {
    ...
    const y = mod2(u1 * Dy);
    const t = mod2(x * y);
    if (!isValid || isNegativeLE(t, P) || y === _0n6) throw new Error(emsg);
...
var SolanaError = class extends Error {
  ...
  constructor(...[code, contextAndErrorOptions]) {
    ...
    if (contextAndErrorOptions) {
      const _a = contextAndErrorOptions, {
        cause
      } = _a, contextRest = __objRest(_a, [
        "cause"
      ]);
      if (cause) {
        errorOptions = {
...
for (let round = 0, R = _1n7, x = 1, y = 0; round < 24; round++) {
  ...
  for (let j = 0; j < 7; j++) {
    ...
    if (R & _2n4) t ^= _1n7 << (_1n7 << BigInt(j)) - _1n7;
  }
  _SHA3_IOTA.push(t);
}
...
var DER = {
  ...
  _int: {
    encode(num2) {
      ...
      if (num2 < _0n8) throw new E("integer: negative integers are not allowed");
...
function weierstrassPoints(opts) {
  ...
  function normPrivateKeyToScalar(key) {
    ...
    if (wrapPrivateKey) num2 = mod(num2, N);
  ...
  class Point2 {
    constructor(px, py, pz) {
      ...
      if (py == null || !Fp2.isValid(py) || Fp2.is0(py)) throw new Error("y required");
    ...
    multiplyUnsafe(sc) {
      ...
      const I = Point2.ZERO;
      if (sc === _0n8) return I;
      if (this.is0() || sc === _1n8) return this;
      if (!endo || wnaf.hasPrecomputes(this)) return wnaf.wNAFCachedUnsafe(this, sc, Point2.normalizeZ);
      let {
        k1neg,
        k1,
        k2neg,
      ...
      while (k1 > _0n8 || k2 > _0n8) {
        if (k1 & _1n8) k1p = k1p.add(d);
        if (k2 & _1n8) k2p = k2p.add(d);
        d = d.double();
        k1 >>= _1n8;
        k2 >>= _1n8;
      }
...
function weierstrass(curveDef) {
  ...
  const {
    ...
    fromBytes(bytes) {
      ...
      if (len === compressedLen && (head === 2 || head === 3)) {
        ...
        if (!inRange(x, _1n8, Fp2.ORDER)) throw new Error("Point is not on curve");
  ...
  class Signature {
    constructor(r, s, recovery) {
      aInRange("r", r, _1n8, CURVE_ORDER);
      aInRange("s", s, _1n8, CURVE_ORDER);
      this.r = r;
      this.s = s;
...
function SWUFpSqrtRatio(Fp2, Z) {
  const q = Fp2.ORDER;
  let l = _0n8;
  for (let o = q - _1n8; o % _2n5 === _0n8; o /= _2n5) l += _1n8;
  const c1 = l;
  const _2n_pow_c1_1 = _2n5 << c1 - _1n8 - _1n8;
  const _2n_pow_c1 = _2n_pow_c1_1 * _2n5;
  const c2 = (q - _1n8) / _2n_pow_c1;
  const c3 = (c2 - _1n8) / _2n5;
  const c4 = _2n_pow_c1 - _1n8;
  const c5 = _2n_pow_c1_1;
  const c6 = Fp2.pow(Z, c2);
  const c7 = Fp2.pow(Z, (c2 + _1n8) / _2n5);
  let sqrtRatio = (u, v) => {
    let tv1 = c6;
...
function createCurve(curveDef, defHash) {
  const create2 = (hash) => weierstrass(__spreadValues(__spreadValues({}, curveDef), getHash(hash)));
  return __spreadProps(__spreadValues({}, create2(defHash)), {
    create: create2
  });
}
...
var Fpk1 = Field(secp256k1P, void 0, void 0, {
  sqrt: sqrtMod
});
...
function schnorrSign(message, privateKey, auxRand = randomBytes(32)) {
  ...
  if (k_ === _0n9) throw new Error("sign failed: k is zero");
  const {
    bytes: rx,
    scalar: k
...
function schnorrVerify(signature2, message, publicKey2) {
  ...
  try {
    ...
    if (!inRange(r, _1n9, secp256k1P)) return false;
    const s = num(sig.subarray(32, 64));
    if (!inRange(s, _1n9, secp256k1N)) return false;
...
var secp256k1_hasher = (() => createHasher2(secp256k1.ProjectivePoint, (scalars) => {
  const {
    x,
    y
